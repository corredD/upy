...
"""
    ...
    This file git_upy/blender/v249/uvcalc_smart_project.py is part of upy.
...
def doit(obj=None):
    ...
    # Create the variables.
    USER_PROJECTION_LIMIT = 66#Draw.Create(66)
    USER_ONLY_SELECTED_FACES = 0#Draw.Create(1)
    USER_SHARE_SPACE = 1#Draw.Create(1) # Only for hole filling.
    USER_STRETCH_ASPECT = 1#Draw.Create(1) # Only for hole filling.
    USER_ISLAND_MARGIN = 0.0#Draw.Create(0.0) # Only for hole filling.
    USER_FILL_HOLES = 0#Draw.Create(0)
    USER_FILL_HOLES_QUALITY = 50#Draw.Create(50) # Only for hole filling.
    USER_VIEW_INIT = 0#Draw.Create(0) # Only for hole filling.
    USER_AREA_WEIGHT = 1#Draw.Create(1) # Only for hole filling.
...
#    USER_AREA_WEIGHT = USER_AREA_WEIGHT.val
    ...
    if is_editmode:
        Window.EditMode(0)
    # Assume face select mode! an annoying hack to toggle face select 
    # mode because Mesh dosent like faceSelectMode.
    ...
    for ob in obList:
        ...
        if USER_VIEW_INIT:
            # Generate Projection
            projectVecs = [Vector(Window.GetViewVector()) * ob.matrixWorld.copy().invert().rotationPart()] 
